...
import { useRealtimeSTTFixed } from '../hooks/useRealtimeSTTFixed';
import { cn } from '@/lib/utils';

interface RealtimeSTTControlsFixedProps {
  isCallActive: boolean;
}

export function RealtimeSTTControlsFixed({ isCallActive }: RealtimeSTTControlsFixedProps) {
  const [activeSpeaker, setActiveSpeaker] = useState<'customer' | 'counselor' | null>(null);

  const customerSTT = useRealtimeSTTFixed({ speaker: 'customer' });
  const counselorSTT = useRealtimeSTTFixed({ speaker: 'counselor' });
